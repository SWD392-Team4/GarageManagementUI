...
export default function CreatePackageServicePage() {
  const { register, handleSubmit, setValue, control, watch } = useForm({
    defaultValues: {
      PackageConditions: [],
      ServiceList: [],
      imagePackage: [] // Lưu danh sách ảnh dưới dạng File
    }
  });
  ...
  const selectedConditions = watch("PackageConditions").map((c) => c.ConditionType).filter(Boolean);
  ...
  const getAvailableConditions = (currentCondition) => {
    return conditionTypes.filter((c) => !selectedConditions.includes(c) || c === currentCondition);
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <select {...register("ServiceCategory")} className="border p-2 w-full">
          ...
          {serviceCategories.map((service, index) => (
            <option key={index} value={service.serviceCategory}>{service.serviceCategory}</option>
          ))}
        </select>
        ...
        <input {...register("PackageName")} placeholder="Package Name" className="border p-2 w-full" />
        ...
        <input {...register("PackagePrice")} placeholder="Package Price" type="number" className="border p-2 w-full" />
        <input {...register("ValidityPeriod")} placeholder="Validity Period" type="number" className="border p-2 w-full" />
        ...
        <input {...register("UsageLimit")} placeholder="Usage Limit" type="number" className="border p-2 w-full" />
        ...
        <div>
          ...
          {fields.map((item, index) => (
            <div key={item.id} className="flex space-x-2 mb-2">
              ...
              <input {...register(`PackageConditions.${index}.ConditionValue`)} placeholder="ConditionValue" type="text" className="border p-2 w-full" />
              <button type="button" onClick={() => remove(index)} className="bg-red-500 text-white px-2">-</button>
            </div>
          ))}
          <button
            type="button"
            onClick={() => append({ ConditionType: "", Value: "" })}
        ...
        {/* Chọn các service */}
        <div>
          <button type="button" onClick={() => setServiceModalOpen(true)} className="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4">
            Select Service
          </button>
          <SelectServiceModal isOpen={isServiceModalOpen} onClose={() => setServiceModalOpen(false)} onSelect={handleServiceSelection} selectedServices={selectedServices} />
        </div>

        {/* Chọn ảnh */}
        <div>
          <button type="button" onClick={() => setImageModalOpen(true)} className="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4">
            Select Images
          </button>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              {selectedImages.map((file, index) => (
                <div key={index} className="border p-2 rounded-lg relative">
                  ...
                  <button
                    onClick={() => setValue("imagePackage", selectedImages.filter((_, i) => i !== index))}
          ...
          <SelectImageModal isOpen={isImageModalOpen} onClose={() => setImageModalOpen(false)} onSelect={handleImageSelection} />
        </div>

        {/* Submit  */}
        <button type="submit" className="bg-blue-500 text-white px-4 py-2 w-full rounded-lg">Tạo gói dịch vụ</button>
      </form>
    </div>
  );
}