...
export default function ModelSelectCarModel({ isOpen, onClose, selectedCarModels, setSelectedCarModels }) {
      ...
      <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-lg w-2/3 max-h-[80vh] flex flex-col">
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">{t("create_product.select_car_models.title")}</h2> {/* ✅ Dịch tiêu đề */}
          ...
          <input
            type="text"
            placeholder={t("create_product.select_car_models.search_placeholder")} // ✅ Dịch placeholder
          ...
          <div className="overflow-y-auto flex-grow grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredCarModels.length > 0 ? (
              filteredCarModels.map((model) => {
                const isSelected = selectedCarModels.some((m) => m.id === model.id);
                return (
                  <div
                    key={model.id}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <img
                        src={model.brandLinkLogo || "https://via.placeholder.com/50"}
                        alt={model.brandName}
                    ...
                    {/* Thông tin chi tiết */}
                    <p className="text-sm text-gray-600">{t("create_product.select_car_models.brand")}: {model.brandName}</p>
                    <p className="text-sm text-gray-600">{t("create_product.select_car_models.category")}: {model.carCategory}</p>
                    <p className="text-sm font-semibold text-blue-700">{t("create_product.select_car_models.year")}: {new Date(model.modelYear).getFullYear()}</p>

                    {/* Trạng thái */}
                    <p className={`text-sm font-semibold mt-1 ${model.status === "Active" ? "text-green-600" : "text-red-600"}`}>
                      {model.status === "Active" ? t("create_product.select_car_models.active") : t("create_product.select_car_models.inactive")}
                    </p>
                  </div>
                );
              })
            ) : (
              <p className="text-gray-500 text-center col-span-full">{t("create_product.select_car_models.no_results")}</p>
            )}
          </div>
