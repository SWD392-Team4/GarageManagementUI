...
export const getAllGoodReceived = async (PageNumber = 1) => {
  try {
    const response = await userService.sendAjax(
      `/api/goods-received/warehouse/${sGoodsReceivedGara.value.garaCurrent}?PageNumber=${PageNumber}`,
      "GET",
      null,
      true
    );
...
export const searchGoodsReceived = async (params) => {
  try {
    ...
    const response = await userService.sendAjax(
      `/api/goods-received/warehouse/${sGoodsReceivedGara.value.garaCurrent}?${queryString}`,
      "GET",
      null,
      true
    );
...
export const SupplierContact = async () => {
  try {
    const response = await userService.sendAjax(
      `/api/supplier/contacts?PageSize=0`,
      "GET",
      null,
      true
    );
