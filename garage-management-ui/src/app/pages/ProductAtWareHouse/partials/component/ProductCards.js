...
import { getStatusLabel, getStatusColor, getStockLabel, getStockColor } from "../../schemas/InventorySchemas"
            ...
            <div className="mt-3 flex flex-col gap-2">
                ...
                <div className="flex justify-between text-sm text-gray-600">
                    <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium">{product.productCategory || "Không có danh mục"}</span>
                    </div>
                    <div className="flex items-center gap-1">
                        <FaIndustry className="text-gray-500" />
                        <span className="font-medium">{product.brandName || "Không có thương hiệu"}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between text-sm text-gray-600">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="flex items-center gap-1">
                            ...
                            <span className="font-semibold text-md">{product.totalQuantity}</span>
                        </span>
                        <span className={`px-3 py-1 text-sm font-semibold rounded-md ${getStockColor(product.totalQuantity)}`}>
                ...
                <span className={`px-3 py-1 text-sm font-semibold rounded-md w-fit ${getStatusColor(product.status)}`}>
