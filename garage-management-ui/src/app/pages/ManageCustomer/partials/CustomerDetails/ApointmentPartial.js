...
import { getApointmentCustomer, getApointmentDetails, searchApointmentCustomer } from '../../services/CustomerService';
...
export default function ApointmentPartial({ customer }) {
    ...
    console.log("check garrage id:", selectedGarageId);


    const fetchListApointment = useCallback(async (page = 1, params = null) => {
        if (selectedGarageId == null) return;

        let response;
        // customer?.phoneNumber,
        if (params) {
            response = await searchApointmentCustomer(selectedGarageId, customer.email, { ...params, PageNumber: page });
        } else {
            response = await getApointmentCustomer(selectedGarageId, customer.email, page,);
        }
    ...
    const columns = useMemo(
        () => [
            { header: t("manage_customer.apointment_info.id"), accessorKey: "id", accessorFn: (_row, index) => index + 1, },
            { header: t("manage_customer.apointment_info.customerName"), accessorKey: "customerName" },
            { header: t("manage_customer.apointment_info.customerPhoneNumber"), accessorKey: "customerPhoneNumber" },
            { header: t("manage_customer.apointment_info.customerEmail"), accessorKey: "customerEmail" },
            { header: t("manage_customer.apointment_info.carLicensePlateNumber"), accessorKey: "carLicensePlateNumber" },
            { header: t("manage_customer.apointment_info.price"), accessorKey: "price" },
            { header: t("manage_customer.apointment_info.status"), accessorKey: "status" },
            { header: t("manage_customer.apointment_info.createdAt"), accessorKey: "createdAt" },
            { header: t("manage_customer.apointment_info.updatedAt"), accessorKey: "updatedAt" },
        ],
        [t]
    );
    const actions = [
    ...
    useEffect(() => {
        console.log("🔥 Checking apointmentDetails before opening modal: ", apointmentDetails);
            ...
            {isUpdateModalOpen && apointmentDetails && (
                <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex justify-center items-center p-4">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-5xl max-h-[90vh] overflow-auto">
                        <h2 className="text-2xl font-semibold mb-4 text-center">Appointment Details</h2>

                        {/* 🔹 Thông tin chung */}
                        <div className="grid grid-cols-2 gap-4 border-b pb-4 mb-4">
                            <p><strong>Garage:</strong> {apointmentDetails.garageId}</p>
                            <p><strong>Car Model:</strong> {apointmentDetails.carModelId}</p>
                            <p><strong>License Plate:</strong> {apointmentDetails.carLicensePlateNumber}</p>
                            <p><strong>Mileage:</strong> {apointmentDetails.mileage}</p>
                            <p><strong>Customer:</strong> {apointmentDetails.customerName}</p>
                            <p><strong>Phone:</strong> {apointmentDetails.customerPhoneNumber}</p>
                            <p><strong>Email:</strong> {apointmentDetails.customerEmail}</p>
                            <p><strong>Verification Code:</strong> {apointmentDetails.verificationCode}</p>
                            <p><strong>Appointment Type:</strong> {apointmentDetails.appointmentType}</p>
                            <p><strong>Status:</strong> {apointmentDetails.status}</p>
                            <p><strong>Estimated Time:</strong> {apointmentDetails.estimatedAppointmentTime}</p>
                            <p><strong>End Time:</strong> {apointmentDetails.estimatedEndTime}</p>
                            <p><strong>Approved At:</strong> {apointmentDetails.approvedAt}</p>
                            <p><strong>Created At:</strong> {apointmentDetails.createdAt}</p>
                            <p><strong>Updated At:</strong> {apointmentDetails.updatedAt}</p>
                            <p><strong>Total Price:</strong> {apointmentDetails.price}</p>
                        </div>

                        {/* 🔹 Danh sách dịch vụ */}
                        {apointmentDetails.appointmentDetails && apointmentDetails.appointmentDetails.length > 0 && (
                            <div className="mt-4">
                                    ...
                                    <tbody>
                                        {apointmentDetails.appointmentDetails.map((service) => (
                                            <tr key={service.serviceHistoryId} className="border">
                                                ...
                                                <td className="border p-2">{service.estimatedHours}</td>
                        ...
                        {apointmentDetails.appointmentDetails &&
                            apointmentDetails.appointmentDetails.some(service => service.appointmentReplacementParts.length > 0) && (
                                <div className="mt-4">
